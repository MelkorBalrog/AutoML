...
"""Tests confirming governance toolbox visibility and functionality."""
...
class GovernanceDiagram(ttk.Frame):
    """Minimal governance diagram recording toolbox calls."""
...
class TestGovernanceVisibility:
    """Grouped tests for governance toolbox in detached windows."""

    def test_toolbox_packed_and_call_sequence(self) -> None:
        try:
            root, _win, clone = _detach_governance_diagram()
        except tk.TclError:
            pytest.skip("Tk not available")
        assert clone.toolbox.winfo_manager() == "pack"
        assert clone.calls == ["rebuild", "switch", "activate"]
        root.destroy()
