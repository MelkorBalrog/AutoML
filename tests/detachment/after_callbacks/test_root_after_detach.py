...
"""Regression tests for root-level after callbacks during detachment."""
...
class TestRootAfterCallbacks:
    """Grouped tests for root-level after callbacks when detaching."""

    class AnimatedButton(tk.Button):
        def __init__(self, master: tk.Widget) -> None:
            super().__init__(master, text="Go")
            self._pulse_after = self._root().after(1, self._pulse)
