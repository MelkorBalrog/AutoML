...
"""Tests for adding already-moved widgets to ``DetachedWindow``."""
...
class DummyDiagram(tk.Frame):
    """Diagram stub exposing toolbox and lifecycle hooks."""
...
class TestDetachedWindowMovedWidget:
    def test_hooks_invoked_for_moved_widget(self) -> None:
        try:
            root = tk.Tk()
        except tk.TclError:
        ...
        # Simulate that the diagram was already moved to the notebook
        win.nb.add(diagram, text="Tab")
        diagram.toolbox.pack_forget()
        win.add_moved_widget(diagram, "Tab")
