...
class TestReparentWidget:
    def test_uses_tk_reparent(self, monkeypatch) -> None:
        ...
        class DummyTk:
            def call(self, *args):
                calls.append(args)
                if args[0] in {"::tk::unsupported::ReparentWindow", "rename"}:
                    return
        ...
        class DummyWidget:
            ...
            def winfo_name(self) -> str:
                return self._w.split(".")[-1]

            def winfo_children(self):  # pragma: no cover - no children in test
                return []

        w = DummyWidget(".w")
        p = DummyWidget(".p")
        reparent_widget(w, p)
        assert any(c[0] == "::tk::unsupported::ReparentWindow" for c in calls)

    def test_renames_path_and_updates_master(self) -> None:
        class DummyTk:
        ...
        class DummyWidget:
            def __init__(self, name: str) -> None:
                self.tk = DummyTk()
                self._w = name
                self.master: DummyWidget | None = None
        ...
        p = DummyWidget(".p")
        reparent_widget(w, p)
        assert ("rename", ".w", ".p.w") in w.tk.calls
        assert w._w == ".p.w"
        assert w.master is p
