...
from analysis.fmeda_utils import (
    ASIL_TARGETS,
    GATE_NODE_TYPES,
    compute_fmeda_metrics as _compute_fmeda_metrics,
)
...
class SafetyAnalysis_FTA_FMEA(FTASubApp, FMEAService, FMEDAManager):
    ...
    def update_hazard_list(self) -> None:
        ...
        self._collect_hazards_from_docs(self.app.hara_docs, hazards, severity_map)
        self._collect_hazop_hazards(hazards, severity_map)
    ...
    def _collect_hazards_from_docs(
        self, docs: Iterable, hazards: list[str], severity_map: dict[str, int]
    ) -> None:
        for doc in docs:
            for e in doc.entries:
                ...
                if h not in hazards:
                    hazards.append(h)
                self._update_severity_map(severity_map, h, getattr(e, "severity", None))

    def _collect_hazop_hazards(
        self, hazards: list[str], severity_map: dict[str, int]
    ) -> None:
        for doc in self.app.hazop_docs:
            for e in doc.entries:
                ...
                if h not in hazards:
                    hazards.append(h)
                if h not in severity_map:
                    self._update_severity_map(
                        severity_map, h, getattr(e, "severity", None)
                    )

    @staticmethod
    def _update_severity_map(
        severity_map: dict[str, int], hazard: str, severity: int | str | None
    ) -> None:
        if severity is None:
            return
        try:
            severity_map[hazard] = int(severity)
        except Exception:
            severity_map[hazard] = 1
