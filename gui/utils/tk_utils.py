...
def cancel_after_events(widget: tk.Widget, cancelled: set[str] | None = None) -> None:
    ...
    # Finally, purge any remaining Tcl command stubs related to animations that
    # may not have been associated with ``after`` identifiers.  These command
    # names typically end with ``_animate`` and would otherwise trigger
    # "invalid command name" errors once the widget has been detached.
    try:
        root = widget._root()
        tcl_cmds = getattr(root, "_tclCommands", None)
        if tcl_cmds:
            for cmd in list(tcl_cmds):
                if cmd.endswith("_animate"):
                    try:
                        root.deletecommand(cmd)
                    except Exception:
                        pass
...
def reparent_widget(widget: tk.Widget, new_parent: tk.Widget) -> None:
    ...
    # First attempt to reparent using Tk's internal command so the toolkit
    # updates its widget hierarchy correctly.  If the command is missing we
    # fall back to the platform's native windowing API.  ``SetParent`` on
    # Windows only updates the OS window relationship; we still need to rename
    # the widget path so Tk reflects the new parent and future commands target
    # the correct window.
    reparented = False
    for cmd in (
        ...
        try:
            tkapp.call(cmd, widget._w, new_parent._w)
            reparented = True
            break
        except tk.TclError:
            continue
    ...
    if not reparented:
        ...
        if sys.platform.startswith("win"):
            ...
            raise tk.TclError("Tk reparenting not supported")

    # Rename the widget path so Tk's internal hierarchy mirrors the new
    # container.  Updating ``_w`` ensures future Tk commands issued by Tkinter
    # reference the renamed path.  Children are recursively updated as their
    # path names change when the parent is renamed.
    new_path = f"{new_parent._w}.{widget.winfo_name()}"
    tkapp.call("rename", widget._w, new_path)
    widget.master = new_parent
    widget._w = new_path

    def _update_paths(parent: tk.Widget) -> None:
        for child in parent.winfo_children():
            child._w = f"{parent._w}.{child.winfo_name()}"
            _update_paths(child)

    _update_paths(widget)